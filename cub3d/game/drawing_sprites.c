...
static int		print_pixel_of_sprite(t_all *s, int j, int i, int y)
{
	...
	if (s->sprite[i].sprite_screen_size_coor == s->win->y || s->sprite[i].sprite_screen_size_coor == 0)
		return(s->fd->i + 1);
	offset_x = floor((double)j * ((double)s->sprite[i].width /\
									s->sprite[i].sprite_width));
	y_sprite = floor(y + ((s->sprite[i].sprite_screen_size_full -\
							s->sprite[i].sprite_screen_size_coor) / 2));
